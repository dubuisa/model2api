...
def configure_logging():
    ...
    def correlation_id_filter(record):
        endpoint_to_exclude = ["health"]

        if any([e in record["message"] for e in endpoint_to_exclude]):
            return False
    ...
    logger.add(
        sink=sys.stdout,
        level=logging.INFO,
        format="<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | <level>{level: <8}</level> | <cyan>{name}</cyan>:<cyan>{function}</cyan>:<cyan>{line}</cyan> | CORRELATION_ID={correlation_id} - <level>{message}</level>",
